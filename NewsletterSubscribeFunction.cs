...
{
    private static readonly HashSet<string> AllowedOrigins = ["https://redflags.cz", "https://www.redflags.cz"];
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", "options")] HttpRequest req)
    {
        ...
        if (req.Method.Equals("OPTIONS", StringComparison.OrdinalIgnoreCase))
            return AllowedOrigins.Contains(origin)
                ? Cors(origin, new StatusCodeResult(204))
        ...
        if (IsRateLimited(clientIp))
            return Cors(origin, new ObjectResult(new { ok = false, error = "Too many requests. Try again later." }) { StatusCode = 429 });

        if (req.ContentLength > MaxBodySize)
            return Cors(origin, new BadRequestObjectResult(new { ok = false, error = "Request too large." }));
        ...
        {
            return Cors(origin, new BadRequestObjectResult(new { ok = false, error = "Invalid request body." }));
        }

        if (string.IsNullOrWhiteSpace(email) || !MailAddress.TryCreate(email, out _))
            return Cors(origin, new BadRequestObjectResult(new { ok = false, error = "Invalid email address." }));
        ...
        if (string.IsNullOrEmpty(apiKey))
        {
            ...
            return Cors(origin, new ObjectResult(new { ok = false, error = "Server misconfiguration." }) { StatusCode = 500 });
        }
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
                return Cors(origin, new OkObjectResult(new { ok = true }));

            var errorBody = await response.Content.ReadAsStringAsync();
            logger.LogWarning("Ecomail API error {StatusCode}: {Body}", (int)response.StatusCode, errorBody);
            return Cors(origin, new ObjectResult(new { ok = false, error = "Subscription failed." }) { StatusCode = 500 });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error calling Ecomail API");
            return Cors(origin, new ObjectResult(new { ok = false, error = "Internal server error." }) { StatusCode = 500 });
        }
    }
    ...
    private static IActionResult Cors(string origin, IActionResult inner)
    {
        return new CorsWrappedResult(origin, inner);
    }

    private class CorsWrappedResult(string origin, IActionResult inner) : IActionResult
    {
